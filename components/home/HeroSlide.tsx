...
export function HeroSlide({ slide, isActive, preload = false }: HeroSlideProps) {
  ...
  const ctaHref = slide.ctaHref.startsWith("/") ? "#newsletter" : slide.ctaHref;
  ...
  return (
    <article
      ...
      className={cn("relative isolate h-full overflow-hidden", slide.backgroundClass)}
    >
      ...
      {slide.accentClass ? (
        <div
          aria-hidden
          className={cn(
            "pointer-events-none absolute top-1/3 -right-20 hidden h-[34rem] w-[34rem] rounded-full blur-3xl md:block",
            slide.accentClass,
          )}
        />
      ) : null}
      ...
      <Container
        size="wide"
        className={cn(
          "relative grid h-full min-h-0 items-center gap-5 py-8 sm:gap-7 sm:py-10 lg:py-14",
          "md:grid-cols-[5fr_7fr] md:gap-10 lg:gap-16",
        )}
      >
        {/* -------- Text column -------- */}
        <motion.div
          ...
          className="text-ink-900 relative z-10 flex flex-col gap-3 sm:gap-4 md:gap-5"
        >
          ...
          <motion.h2
            variants={safeText}
            className={cn(
              "font-display leading-[1.05] font-medium tracking-tight",
              "text-3xl text-balance sm:text-5xl lg:text-6xl",
            )}
          >
            {slide.title}
          </motion.h2>
          ...
          <motion.p
            variants={safeText}
            className="text-ink-700 max-w-xl text-sm leading-relaxed text-pretty sm:text-base lg:text-lg"
          >
            {slide.description}
          </motion.p>
        ...
        <motion.div
          ...
          animate={isActive ? "visible" : "hidden"}
          variants={safeScene}
          className="rounded-card relative h-[clamp(10rem,28dvh,17rem)] w-full overflow-hidden md:h-[64dvh] md:max-h-[44rem] lg:h-[68dvh] lg:max-h-[46rem]"
        >
          <Image
            src={slide.image.src}
            alt={slide.image.alt}
          ...
          <div
            aria-hidden
            className="from-ink-900/15 pointer-events-none absolute inset-0 bg-gradient-to-tr via-transparent to-transparent"
          />
        </motion.div>
      </Container>
    </article>
