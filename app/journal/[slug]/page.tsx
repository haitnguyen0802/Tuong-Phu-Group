...
const JOURNAL_BUILD_PLACEHOLDER_SLUG = "__build_placeholder__";

export async function generateStaticParams() {
  const articles = await getArticlesData();
  if (!articles.length) {
    return [{ slug: JOURNAL_BUILD_PLACEHOLDER_SLUG }];
  }
...
export async function generateMetadata({ params }: ArticleDetailPageProps): Promise<Metadata> {
  ...
  if (slug === JOURNAL_BUILD_PLACEHOLDER_SLUG) {
...
export default async function ArticleDetailPage({ params }: ArticleDetailPageProps) {
  const { slug } = await params;
  if (slug === JOURNAL_BUILD_PLACEHOLDER_SLUG) notFound();

  const [article, footer] = await Promise.all([getArticleDetailData(slug), fetchFooter()]);
  if (!article) notFound();
